...
class Batch(Data):
    r"""A numpy replica of the plain old python object modeling a batch of graphs as one big
    ...
    def from_data_list(cls, data_list, follow_batch=[], exclude_keys=[]):
        ...
        for i, data in enumerate(data_list):
            for key in keys:
                ...
                if isinstance(item, np.ndarray) and item.ndim == 0: ## which means this tensor is a scalar
                    cat_dim = None
                cat_dims[key] = cat_dim

                # Add a batch dimension to items whose `cat_dim` is `None`:
                if isinstance(item, np.ndarray) and cat_dim is None:
                    cat_dim = 0  # Concatenate along this new batch dimension.
                    item = item[None,...]
                    # device = item.device
                elif isinstance(item, np.ndarray): ## cat_dim is not none
                    size = item.shape[cat_dim]
                    # device = item.device
                
                # pending np's support for nested data type
            ...
            if num_nodes is not None:
                item = np.full((num_nodes,), i, dtype=np.int64)
        ...
        for key in batch.keys:
            ...
            if isinstance(item, np.ndarray):
                ...
                # suspend support for nested datatype
                batch[key] = np.concatenate(items, cat_dim)
                
            elif isinstance(item, (int, float)):
                batch[key] = np.array(items)
    ...
    def get_example(self, idx: int) -> Data:
        ...
        def narrow_np(arr, dim, start, length):
